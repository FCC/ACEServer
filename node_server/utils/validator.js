...
module.exports = {
    ...
    isPasswordComplex: function (password) {
        let isFormatCorrect = false;
        if (password) {
            let re = /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,64}$/;
    ...
    isUsernameValid: function (username) {
        ...
        if (username) {
            ...
            isFormatCorrect = ((username.length >= 4) && (username.length <= 15) && (legalChars.test(username)));
    ...
    isNameValid: function (name) {
        let isFormatCorrect = false;
        if (name) {
            let legalChars = /^[a-zA-Z]+$/; // allow letters only
